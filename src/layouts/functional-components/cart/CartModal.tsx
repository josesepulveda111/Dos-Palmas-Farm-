                    ...
                    <div className="relative flex w-full flex-row justify-between px-1 py-4">
                      ...
                      <a href={merchandiseUrl.toString()} className="z-30 flex flex-row space-x-4">
                        <div className="relative h-16 w-16 overflow-hidden rounded-md border border-neutral-300 bg-neutral-300">
                          {(() => {
                            const images = item?.merchandise?.product?.images;
                            const imagesArray = Array.isArray(images)
                              ? images
                              : Array.isArray(images?.edges)
                              ? images.edges.map((e: any) => e.node)
                              : [];
                            const color = item?.merchandise?.selectedOptions?.find((o: any) => o?.name === "Color")?.value;
                            const match = imagesArray.find((img: any) => img?.altText === color);
                            const src = match?.url || item?.merchandise?.product?.featuredImage?.url || "/images/product_image404.jpg";
                            return (
                              <img className="h-full w-full object-cover" src={src} alt={item.merchandise.title} width={64} height={64} />
                            );
                          })()}
                        </div>
